...
create table OwnedBy (
    ...
    CONSTRAINT OwnedBy_regNum_fk FOREIGN KEY (regNum) REFERENCES RaceHorse (regNum) on DELETE CASCADE,
    CONSTRAINT OwnedBy_pId_fk FOREIGN KEY (pId) REFERENCES Person (pId) on DELETE CASCADE
);
...
create table Entry (
    ...
    CONSTRAINT Entry_schId_rNumber_fk FOREIGN KEY (schId, rNumber) REFERENCES Race (schId, rNumber) on DELETE CASCADE,
    CONSTRAINT Entry_jockey_fk FOREIGN KEY (jockey) REFERENCES Person (pId) on DELETE SET NULL,
    CONSTRAINT Entry_horse_fk FOREIGN KEY (horse) REFERENCES RaceHorse (regNum) on DELETE CASCADE
);
